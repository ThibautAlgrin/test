...
use Voryx\RESTGeneratorBundle\Mocker\FactoryMocker;
...
class DoctrineRESTGenerator extends Generator
{
    ...
    public function generate(BundleInterface $bundle, $entity, ClassMetadataInfo $metadata, $routePrefix, $forceOverwrite)
    {
        ...
        $this->className['entity'] = $entity;
        $this->namespace['entity'] = $bundle->getNamespace().'\\Entity';
        $this->namespace['controller'] = $bundle->getNamespace().'\\Controller';
        $this->className['controller'] = $entity.'Controller';
        $this->namespace['test'] = $bundle->getNamespace().'\\Test';
        $this->className['test'] = $entity.'ControllerTest';
        $this->entity   = $entity;
        $this->bundle   = $bundle;
    ...
    protected function getRoutes(array $collection) {
        $routes = [];
        foreach ($collection as $name => $route) {
            $tmp = explode('_', $name);
            switch ($tmp[0]) {
                case 'get':
                    $n = substr($tmp[1], strlen($tmp[1]) - 1) == 's' ? 'all' : 'one';
                    $routes[$n] = $name;
                    break;
                default:
                    $routes[$tmp[0]] = $name;
    ...
    public function generateTestClass()
    {
        ...
        $entityNamespace = implode('\\', $parts);
        $controller = sprintf(
            '%s\\Controller\\%s%sRESTController',
            $this->bundle->getNamespace(),
            str_replace('\\', '/', $entityNamespace),
            $entityClass
        );
        /** @var \FOS\RestBundle\Routing\RestRouteCollection $collection */
        $collection = $this->routing->read(new \ReflectionClass($controller));
        $routes = $this->getRoutes($collection->all());
    ...
    /**
     * @param FactoryMocker $factoryMockers
     */
    public function setFactoryMockers(FactoryMocker $factoryMockers)
    {
        $this->factoryMockers = $factoryMockers;
    }
}
